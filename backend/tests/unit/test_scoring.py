...
class TestMultipleStockScoring:
    """Test scoring multiple stocks functionality."""
    
    def test_score_multiple_stocks_success(self, config_manager):
        """Test successful scoring of multiple stocks."""
        # Mock data
        all_data = {
            'prices': pd.DataFrame({
                'STOCK1': {'close': 100, 'volume': 1000000, 'rsi_14': 55},
                'STOCK2': {'close': 200, 'volume': 500000, 'rsi_14': 45}
            }).T,
            'fundamentals': pd.DataFrame({
                'STOCK1': {'roe_3y': 0.15, 'market_cap_cr': 5000},
                'STOCK2': {'roe_3y': 0.12, 'market_cap_cr': 8000}
            }).T,
            'ownership': pd.DataFrame({
                'STOCK1': {'promoter_holding_pct': 0.6, 'fii_holding_pct': 0.2},
                'STOCK2': {'promoter_holding_pct': 0.55, 'fii_holding_pct': 0.25}
            }).T,
            'sector': pd.DataFrame(),
            'market': pd.DataFrame()
        }
        
        sector_groups = {'STOCK1': 'it', 'STOCK2': 'banks'}
        
        with patch('greyoak_score.core.scoring.calculate_greyoak_score') as mock_score:
            # Mock successful scoring
            mock_score.side_effect = [
                ScoreOutput(
                    ticker="STOCK1", score=75.0, band="Buy", pillars=PillarScores(F=70, T=80, R=75, O=70, Q=85, S=75),
                ...
                ScoreOutput(
                    ticker="STOCK2", score=68.0, band="Hold", pillars=PillarScores(F=75, T=65, R=70, O=65, Q=80, S=70),
                    risk_penalty=3.0, guardrail_flags=["LowDataHold"], confidence=0.78, s_z=0.8,
                    sector_group="banks", mode="trader", as_of=datetime.now(timezone.utc), config_hash="abc123"
                )
            ]
            
            results = score_multiple_stocks(
            ...
            assert results['STOCK1'].score == 75.0
            assert results['STOCK2'].score == 68.0
    
    def test_score_multiple_stocks_with_failures(self, config_manager):
        """Test scoring multiple stocks where some fail."""
        all_data = {
            'prices': pd.DataFrame(),
            'fundamentals': pd.DataFrame(),
            'ownership': pd.DataFrame(),
        ...
        sector_groups = {'STOCK1': 'it', 'STOCK2': 'banks', 'STOCK3': 'metals'}
        
        with patch('greyoak_score.core.scoring.calculate_greyoak_score') as mock_score:
            # Mock one success and one failure
            def mock_score_func(*args, **kwargs):
            ...
            # Only successful stock should be in results
            assert len(results) == 1
            assert 'STOCK1' in results
            assert 'STOCK2' not in results
